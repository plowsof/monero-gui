...
gitrepo_list=(
    ...
    "https://gitlab.freedesktop.org/xorg/lib/libxcb-util" "0.4.0" "acf790d7752f36e450d476ad79807d4012ec863b"
    "https://gitlab.freedesktop.org/xorg/lib/libxcb-image" "0.4.0" "d882052fb2ce439c6483fce944ba8f16f7294639"
    "https://gitlab.freedesktop.org/xorg/lib/libxcb-keysyms" "0.4.0" "0e51ee5570a6a80bdf98770b975dfe8a57f4eeb1"
    "https://gitlab.freedesktop.org/xorg/lib/libxcb-render-util" "0.3.9" "0317caf63de532fd7a0493ed6afa871a67253747"
    "https://gitlab.freedesktop.org/xorg/lib/libxcb-wm" "0.4.1" "24eb17df2e1245885e72c9d4bbb0a0f69f0700f2"
    ...
    "https://git.savannah.gnu.org/git/freetype/freetype2.git" "VER-2-10-2" "132f19b779828b194b3fede187cee719785db4d8"
...
get_repos() {
    ...
    for ((i=0; i<${#repo_list[@]}; i+=3)); do
        local repo_url="${repo_list[i]}"
        local branch_or_tag="${repo_list[i+1]}"
        local commit_hash="${repo_list[i+2]}"
        
        echo "Cloning $(basename "$repo_url")"
        
        git clone -b "$branch_or_tag" --depth 1 "$repo_url" && \
...
build_project() {
  ...
  if [ -f "./bootstrap" ]; then
      ...
      echo "No recognized build script found for $1";
      return 1;
  fi
  make -j$THREADS
  make install
...
build_standard_projects() {
    build_project xorgproto
    build_project xcbproto 
    build_project libxau --enable-shared --disable-static 
    build_project libxcb-util --enable-shared --disable-static 
    build_project libxcb-image --enable-shared --disable-static 
    build_project libxcb-keysyms --enable-shared --disable-static 
    build_project libxcb-render-util --enable-shared --disable-static 
    build_project libxcb-wm --enable-shared --disable-static 
    build_project libxkbcommon --prefix=/usr --enable-shared --disable-static --enable-x11 --disable-docs 
    build_project zlib --static 
    build_project freetype2 --disable-shared --enable-static --with-zlib=no 
    build_project expat-2.4.8 --enable-static --disable-shared --prefix=/usr 
    build_project fontconfig --disable-shared --enable-static --sysconfdir=/etc --localstatedir=/var 
    build_project icu/icu4c/source --disable-shared --enable-static --disable-tests --disable-samples 
    build_project libusb --disable-shared --enable-static 
    build_project hidapi --disable-shared --enable-static 
    build_project libzmq --disable-shared --enable-static --disable-libunwind --with-libsodium 
    build_project libgpg-error --disable-shared --enable-static --disable-doc --disable-tests 
    build_project libgcrypt --disable-shared --enable-static --disable-doc 
    build_project protobuf --enable-static --disable-shared 
    build_project CMake
}
...
build_special_case() {
  ...
  case "$project_dir" in
    ...
    *)
      echo "No special case defined for $project_dir"
...
#seperate container with just git and ca-certificates
#clone_git_repos
